...
export async function createChannel(name, metaFilename) {
  ...
  return channel
    ? channel.id
    : (await client.channel.create({ data: { name, metaFilename } })).id;
}
 ...
 */
export async function addVideo(title, filename, yt_id, channelId, thumbnail) {
  ...
  return video
    ...
    : (
        await client.video.create({
          data: { title, filename, channelId, yt_id, thumbnail },
        })
      ).id;
}
...
export async function getChannel(username, title) {
  ...
  return channel
    ? channel.id
    : (await client.channel.create({ data: { title, username } })).id;
}
